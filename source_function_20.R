...
## The true trajectory behind longitudinal observations is a sine function
## with random coefficients (sin_par and sin_int).
longit.true=function(t,sin_par,sin_int){
  sin_par*sin(t)+sin_int
}
...
##################Generate data where the true trjectory is a sine function
gendat=function(s,obsmax,gammatrue,alpha1true,alpha2true,sigmatrue){
  set.seed(s)
  basetrue=function(t){ifelse(t<2,0,exp(-2))} ## 0 when t<2
  W=sample(c(0,1),size=m,replace = TRUE)
  sin_par=runif(m,min=0.5,max=3)
  sin_int=runif(m,min=2,max=5)
  
  TD.fun=function(i,t){# i is a scalar, t can be a vector
    tt=function(t){
      c(basetrue(t)*exp(gammatrue*W[i]+alpha1true*(sin_par[i]*sin(t)+sin_int[i])+
                          alpha2true*sqrt(sin_par[i]^2*(t/2-sin(2*t)/4))))
    }
  ...
  for (i in 1:m){
    ...
    for(j in c(seq(0,1.6,by=0.4),seq(2,Time[i],by=0.5))){
      YY=longit.true(j,sin_par[i],sin_int[i])+rnorm(1,sd=sigmatrue)
      Y=rbind(Y,YY)
      time=rbind(time,j)
    }
  }
  ...
  return(list(data=data,sin_ran=sin_par,sin_int=sin_int))
}
...
#### (empirical cumulative baseline hazard)
#cumulative baseline hazard function if true survival parameters and true biomarker trajectory and its second derivative are known
cumbase_emp=function(data,data.id,sin_par,sin_int,gammatrue,alpha1true,alpha2true){
  ...
  for(t in sort(unique(data$Time[data$delta==1]))){
    rs.id=riskset(t)
    s=sum(exp(gammatrue*data.id$W[rs.id]+alpha1true*(sin_par[rs.id]*sin(t)+sin_int[rs.id])+
                alpha2true*sqrt(sin_par[rs.id]^2*(t/2-sin(2*t)/4))))
...
cox.nonpar=function(data,data.id,sin_par,sin_int){
  ...
  conloglik_nonpar=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      Ti=data.id$Time[data.id$id==i]
      rs.id=data.id$id[data.id$Time>=Ti]
      conloglik=conloglik+gamma*data.id$W[i]+alpha1*(sin_par[i]*sin(Ti)+sin_int[i])+
        alpha2*sqrt(sin_par[i]^2*(Ti/2-sin(2*Ti)/4))-
        log(sum(exp(gamma*data.id$W[rs.id]+alpha1*(sin_par[rs.id]*sin(Ti)+sin_int[rs.id])+
                      alpha2*sqrt(sin_par[rs.id]^2*(Ti/2-sin(2*Ti)/4)))))
    }
    return(-conloglik)
  }
  ...
  for(t in sort(unique(data$Time[data$delta==1]))){
    rs.id=riskset(t)
    s=sum(exp(gamma*data.id$W[rs.id]+alpha1*(sin_par[rs.id]*sin(t)+sin_int[rs.id])+
                alpha2*sqrt(sin_par[rs.id]^2*(t/2-sin(2*t)/4))))
