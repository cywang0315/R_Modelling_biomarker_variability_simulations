###Simulation study: case 3
###Program in a parallel way, all functions required are stored in "source_function_22.R".
...
sfLibrary(mvtnorm)
sfLibrary(tensor)
sfSource(here::here("source_function_22.R"))

SIMULATE=function(s){
  ##Generate simulation data
  Data=gendat(s,obsmax=6,gammatrue=-1,alpha1true=0.3,alpha2true=0,beta_cub=-1/6,beta_slo=1,
              sigmatrue=0.4)
  data=Data$data
  ...
  knot_1=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_1)){
    return(NULL)
  }
  
  pattern_candi=c(1.5)
  pattern_set[[2]]=pattern_candi
  knot_2=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_2)){
    return(NULL)
  }
  
  pattern_candi=c(0.25)
  pattern_set[[3]]=pattern_candi
  knot_3=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_3)){
    return(NULL)
  }
  
  pattern_candi=quantile(data$time,0.5)
  pattern_set[[4]]=pattern_candi
  knot_4=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_4)){
    return(NULL)
  }
  
  pattern_candi=quantile(data$time,0.75)
  pattern_set[[5]]=pattern_candi
  knot_5=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_5)){
    return(NULL)
  }
  
  pattern_candi=quantile(data$time,c(0.25,0.5,0.75))
  pattern_set[[6]]=pattern_candi
  knot_6=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_6)){
    return(NULL)
  }
  
  pattern_candi=c(3)
  pattern_set[[7]]=pattern_candi
  knot_7=try(knot.sel(data,pattern_candi,ctl=lmeControl (msMaxIter=100)))
  if('try-error' %in% class(knot_7)){
    return(NULL)
  }
  ...
  cox.true=cox.nonpar(data,data.id,beta_cub=-1/6,beta_slo=1,Data$ran_int,Data$ran_slo)
  cox.true$res_par
  ...
  res_non.par=c(cox.true$res_par$par)
